...
class Tooltip:
    ...
    def __init__(self, widget, text_generator, delay=700, follow=True):
        self.widget = widget
        self.text_generator = (
            text_generator  # A function that returns the text to display
        )
...
class QPostViewer:
    ...
    def __init__(self, root):
        ...
        self.bookmarked_posts = utils.load_bookmarks_from_file(
            config.BOOKMARKS_FILE_PATH
        )
        ...
        self.current_theme = self.app_settings.get(
            "theme", settings.DEFAULT_SETTINGS["theme"]
        )
        ...
        self.details_outer_frame.grid(
            row=0, column=1, padx=(0, 10), pady=(10, 0), sticky="nswe"
        )
        ...
        self.post_tree = ttk.Treeview(
            self.tree_frame, columns=("Post #", "Date", "Bookmarked"), show="headings"
        )
        self.scrollbar_y = ttk.Scrollbar(
            self.tree_frame, orient="vertical", command=self.post_tree.yview
        )
        ...
        self.post_text_scrollbar = ttk.Scrollbar(
            self.text_area_frame, orient="vertical", command=self.post_text_area.yview
        )
        ...
        self.notes_frame = ttk.Labelframe(
            self.details_outer_frame, text="User Note", padding=(10, 5)
        )
        ...
        self.note_text_scrollbar = ttk.Scrollbar(
            self.notes_frame, orient="vertical", command=self.note_text_area.yview
        )
        ...
        self.edit_save_note_button = ttk.Button(
            self.notes_frame,
            text="Edit Note",
            command=self.toggle_note_edit_mode,
            width=10,
        )
        self.edit_save_note_button.grid(
            row=1, column=0, columnspan=2, pady=(5, 0), sticky="e"
        )
        ...
        controls_main_frame.grid(
            row=1, column=0, columnspan=2, padx=10, pady=(5, 10), sticky="ew"
        )

        nav_frame = ttk.Frame(controls_main_frame)
        nav_frame.pack(pady=(0, 5), fill=tk.X)
        self.prev_button = ttk.Button(
            nav_frame, text="<< Prev", command=self.prev_post, width=8
        )
        self.prev_button.pack(side="left", padx=2)
        self.post_number_label = ttk.Label(
            nav_frame, text="", width=35, anchor="center", font=("Arial", 11, "bold")
        )
        self.post_number_label.pack(side="left", padx=5, expand=True, fill=tk.X)
        self.next_button = ttk.Button(
            nav_frame, text="Next >>", command=self.next_post, width=8
        )
        self.next_button.pack(side="left", padx=2)

        actions_frame = ttk.Labelframe(
            controls_main_frame, text="Search & Actions", padding=(10, 5)
        )
        ...
        ttk.Button(
            search_fields_frame, text="Go", command=self.search_post_by_number, width=4
        ).pack(side=tk.LEFT, padx=(0, 10))
        self.post_entry.bind(
            "<FocusIn>",
            lambda e, p=config.PLACEHOLDER_POST_NUM: self.clear_placeholder(
                e, p, self.post_entry
            ),
        )
        self.post_entry.bind(
            "<FocusOut>",
            lambda e, p=config.PLACEHOLDER_POST_NUM: self.restore_placeholder(
                e, p, self.post_entry
            ),
        )
        ...
        self.keyword_entry = ttk.Entry(
            search_fields_frame, width=20, font=("Arial", 10)
        )
        self.keyword_entry.pack(side=tk.LEFT, padx=(0, 2), expand=True, fill=tk.X)
        ttk.Button(
            search_fields_frame, text="Go", command=self.search_by_keyword, width=4
        ).pack(side=tk.LEFT)
        self.keyword_entry.bind(
            "<FocusIn>",
            lambda e, p=config.PLACEHOLDER_KEYWORD: self.clear_placeholder(
                e, p, self.keyword_entry
            ),
        )
        self.keyword_entry.bind(
            "<FocusOut>",
            lambda e, p=config.PLACEHOLDER_KEYWORD: self.restore_placeholder(
                e, p, self.keyword_entry
            ),
        )
        ...
        ttk.Button(
            buttons_frame1, text="Search by Date", command=self.show_calendar
        ).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        ttk.Button(
            buttons_frame1, text="Delta Search", command=self.show_day_delta_dialog
        ).pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)

        self.todays_deltas_button = ttk.Button(
            buttons_frame1, text="Today's Deltas", command=self.search_today_deltas
        )
        ...
        self.bookmark_button = ttk.Button(
            ...
            command=self.toggle_current_post_bookmark,
        )
        self.bookmark_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        self.bookmark_button.config(state=tk.DISABLED)
        self.view_bookmarks_button = ttk.Button(
            buttons_frame3,
            text=f"View Bookmarks ({len(self.bookmarked_posts)})",
            command=self.view_bookmarked_gui_posts,
        )
        ...
        ttk.OptionMenu(
            bottom_buttons_frame, self.export_var, "HTML", "HTML", "CSV"
        ).pack(side=tk.LEFT, padx=(0, 5), fill=tk.X)
        ttk.Button(
            bottom_buttons_frame,
            text="Export Displayed List",
            command=self.export_displayed_list,
        ).pack(side=tk.LEFT, expand=True, fill=tk.X)

        self.content_sync_button = ttk.Button(
            bottom_buttons_frame, text="Content Sync", command=self.show_download_window
        )
        self.content_sync_button.pack(side=tk.LEFT, padx=5, fill=tk.X)

        self.settings_button = ttk.Button(
            bottom_buttons_frame, text="Settings", command=self.show_settings_window
        )
        self.settings_button.pack(side=tk.LEFT, padx=5, fill=tk.X)

        self.theme_toggle_button = ttk.Button(
            bottom_buttons_frame, text="Dark to Light", command=self.toggle_theme
        )
        self.theme_toggle_button.pack(side=tk.LEFT, padx=5, fill=tk.X)

        self.help_button = ttk.Button(
            bottom_buttons_frame, text="Help & Info", command=self.show_help_window
        )
        self.help_button.pack(side=tk.LEFT, padx=5, fill=tk.X)

        self.about_button = ttk.Button(
            bottom_buttons_frame, text="About", command=self.show_about_dialog
        )
        self.about_button.pack(side=tk.LEFT, padx=5, fill=tk.X)

        ttk.Button(bottom_buttons_frame, text="Quit App", command=self.on_closing).pack(
            side=tk.LEFT, padx=5, fill=tk.X
        )
        ...
        if self.df_all_posts is not None and not self.df_all_posts.empty:
            ...
            messagebox.showerror(
                "Critical Error",
                "Failed to load any post data. Application cannot continue.",
            )
            self.root.destroy()
    ...
    def on_closing(self):
        ...
        utils.save_user_notes(
            self.user_notes, config.USER_NOTES_FILE_PATH
        )  # Save user notes
        self.root.destroy()
    ...
    def apply_dark_theme(self):
        ...
        heading_bg = "#4f4f4f"
        scrollbar_bg = "#4f4f4f"
        scrollbar_trough = "#3c3f41"
        scrollbar_arrow = "#e0e0e0"

        self.root.configure(bg=bg_color)
        self.style.configure(
            ".", background=bg_color, foreground=fg_color, font=("Arial", 10)
        )
        self.style.configure("TFrame", background=bg_color)
        self.style.configure(
            "TLabel", background=bg_color, foreground=fg_color, padding=3
        )
        self.style.configure(
            "TButton",
        ...
        self.style.map(
            "TButton",
            background=[("active", button_active_bg), ("pressed", "#5a5a5a")],
        ...
        self.style.map(
            ...
            lightcolor=[("readonly", button_bg)],
            darkcolor=[("readonly", button_bg)],
        )

        self.post_text_area.configure(
            bg=entry_bg,
            fg=fg_color,
            insertbackground=fg_color,
        ...
        if hasattr(self, "note_text_area"):
            self.note_text_area.configure(
                bg=entry_bg,
                fg=fg_color,
                insertbackground=fg_color,
                selectbackground=tree_sel_bg,
            )
        if hasattr(self, "image_display_frame"):
            self.image_display_frame.configure(style="TFrame")
        ...
        self.post_text_area.tag_configure(
            "clickable_link_style", foreground=self.link_label_fg_dark
        )
        ...
        if hasattr(self, "theme_toggle_button"):
            self.theme_toggle_button.config(text="Dark to Light")
        if hasattr(self, "post_entry") and self.post_entry.winfo_exists():
            self.restore_placeholder(None, config.PLACEHOLDER_POST_NUM, self.post_entry)
        if hasattr(self, "keyword_entry") and self.keyword_entry.winfo_exists():
            self.restore_placeholder(
                None, config.PLACEHOLDER_KEYWORD, self.keyword_entry
            )
    ...
    def apply_light_theme(self):
        ...
        if hasattr(self, "image_display_frame"):
            self.image_display_frame.configure(style="TFrame")
        self.post_text_area.tag_configure("bold_label", foreground="#333333")
        self.post_text_area.tag_configure("post_number_val", foreground="#D9534F")
        self.post_text_area.tag_configure("date_val", foreground="#5CB85C")
        self.post_text_area.tag_configure("author_val", foreground="#555555")
        self.post_text_area.tag_configure("themes_val", foreground="#8E44AD")
        self.post_text_area.tag_configure("image_val", foreground="#337AB7")
        self.post_text_area.tag_configure(
            "clickable_link_style", foreground=self.link_label_fg_light
        )
        self.post_text_area.tag_configure("bookmarked_header", foreground="#F0AD4E")
        self.post_text_area.tag_configure("quoted_ref_header", foreground="#4A4A4A")
        self.post_text_area.tag_configure("quoted_ref_text_body", foreground="#202020")
        ...
        if hasattr(self, "theme_toggle_button"):
            self.theme_toggle_button.config(text="Into the Dark")
        if hasattr(self, "post_entry") and self.post_entry.winfo_exists():
            self.restore_placeholder(None, config.PLACEHOLDER_POST_NUM, self.post_entry)
        if hasattr(self, "keyword_entry") and self.keyword_entry.winfo_exists():
            self.restore_placeholder(
    ...
    def toggle_theme(self):
        ...
        if self.current_display_idx != -1 and not (
            self.df_displayed is None or self.df_displayed.empty
        ):
            self.update_display()
        else:
            self.show_welcome_message()
    ...
    def configure_text_tags(self):
        ...
        self.post_text_area.tag_configure(
            "bold_label", font=(default_font_name, 11, "bold")
        )
        self.post_text_area.tag_configure(
            "post_number_val", font=(default_font_name, 11, "bold")
        )
        ...
        self.post_text_area.tag_configure(
            "themes_val", font=(default_font_name, 10, "italic")
        )
        ...
        def show_arrow_cursor(event):
            event.widget.config(cursor=self.default_text_area_cursor)

        self.post_text_area.tag_bind(
            "clickable_link_style", "<Enter>", show_hand_cursor
        )
        self.post_text_area.tag_bind(
            "clickable_link_style", "<Leave>", show_arrow_cursor
        )

        self.post_text_area.tag_configure(
            "bookmarked_header", font=(default_font_name, 11, "bold")
        )
        self.post_text_area.tag_configure(
            "quoted_ref_header",
            font=(default_font_name, 10, "italic", "bold"),
            lmargin1=20,
            lmargin2=20,
            spacing1=5,
        )
        self.post_text_area.tag_configure(
            ...
            font=(default_font_name, 10, "italic"),
            lmargin1=25,
            lmargin2=25,
            spacing3=5,
        )
        ...
        self.post_text_area.tag_configure(
            "welcome_emphasis_tag", font=(default_font_name, 10, "italic")
        )
        self.post_text_area.tag_configure(
            "welcome_closing_tag",
            font=(default_font_name, 10, "bold"),
            justify=tk.CENTER,
            spacing1=10,
        )
    ...
    def _prevent_text_edit(self, event):
        ...
        if (
            event.keysym in allowed_nav_keys
            or event.keysym.lower() in ["a"]
            and event.state & 0x0004
        ):  # Allow Ctrl+A
            return
    ...
    def _insert_text_with_clickable_urls(
        self,
        text_content_raw,
        base_tags_tuple,
        main_post_original_df_index,
        link_event_tag_prefix,
    ):
        text_content = utils.sanitize_text_for_tkinter(text_content_raw)

        if not isinstance(text_content, str) or not text_content.strip():
            self.post_text_area.insert(
                tk.END,
                str(text_content) if pd.notna(text_content) else "",
                base_tags_tuple if base_tags_tuple else (),
            )
            return
        ...
        for url_match in config.URL_REGEX.finditer(text_content):
            ...
            self.post_text_area.tag_bind(
                clickable_tag_instance,
                "<Button-1>",
                lambda e, u=url: utils.open_link_with_preference(u, self.app_settings),
            )
            last_end = end
    ...
    def update_display(self):
        ...
        if (
            self.df_displayed is None
            or self.df_displayed.empty
            or not (0 <= self.current_display_idx < len(self.df_displayed))
        ):
            self.show_welcome_message()
            return
        ...
        filename_post_id = str(
            post_number_val if pd.notna(post_number_val) else original_df_index
        )
        ...
        pn_str = (
            f"#{pn_display_raw}"
            if pd.notna(pn_display_raw)
            else f"(Idx:{original_df_index})"
        )
        is_bookmarked = original_df_index in self.bookmarked_posts
        bookmark_indicator_text_raw = "[BOOKMARKED]" if is_bookmarked else ""
        self.post_text_area.insert(
            tk.END,
            utils.sanitize_text_for_tkinter(f"QView Post {pn_str} "),
            "post_number_val",
        )
        if is_bookmarked:
            self.post_text_area.insert(
                tk.END,
                utils.sanitize_text_for_tkinter(bookmark_indicator_text_raw) + "\n",
                "bookmarked_header",
            )
        else:
        ...
        if pd.notna(dt_val):
            ...
            date_local_str = utils.sanitize_text_for_tkinter(
                f"{dt_local.strftime('%Y-%m-%d %H:%M:%S %Z')} (Local)\n"
            )
            date_utc_str = utils.sanitize_text_for_tkinter(
                f"{dt_utc.strftime('%Y-%m-%d %H:%M:%S %Z')} (UTC)\n"
            )
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("Date: "), "bold_label"
            )
            self.post_text_area.insert(tk.END, date_local_str, "date_val")
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("      "), "bold_label"
            )
            self.post_text_area.insert(tk.END, date_utc_str, "date_val")
        else:
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("Date: No Date\n"), "bold_label"
            )
        author_text_raw = post.get("Author", "")
        tripcode_text_raw = post.get("Tripcode", "")
        author_text = utils.sanitize_text_for_tkinter(author_text_raw)
        tripcode_text = utils.sanitize_text_for_tkinter(tripcode_text_raw)
        if author_text and pd.notna(author_text_raw):
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("Author: "), "bold_label"
            )
            self.post_text_area.insert(tk.END, f"{author_text}\n", "author_val")
        if tripcode_text and pd.notna(tripcode_text_raw):
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("Tripcode: "), "bold_label"
            )
            self.post_text_area.insert(tk.END, f"{tripcode_text}\n", "author_val")
        themes_list = post.get("Themes", [])
        if themes_list and isinstance(themes_list, list) and len(themes_list) > 0:
            themes_str = utils.sanitize_text_for_tkinter(f"{', '.join(themes_list)}\n")
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("Themes: "), "bold_label"
            )
            self.post_text_area.insert(tk.END, themes_str, "themes_val")
        ...
        if isinstance(referenced_posts_raw_data, list) and referenced_posts_raw_data:
            self.post_text_area.insert(
                tk.END,
                utils.sanitize_text_for_tkinter("\nReferenced Content:\n"),
                ("bold_label"),
            )
            for ref_idx, ref_post_data in enumerate(referenced_posts_raw_data):
                if not isinstance(ref_post_data, dict):
                    continue
                ref_num_raw = ref_post_data.get("reference", "")
                ref_author_id_raw = ref_post_data.get("author_id")
                ref_text_content_raw = ref_post_data.get(
                    "text", "[No text in reference]"
                )
                ref_num = utils.sanitize_text_for_tkinter(str(ref_num_raw))
                ref_author_id = utils.sanitize_text_for_tkinter(str(ref_author_id_raw))
                header_parts = ["↪ Quoting "]
                if ref_num:
                    header_parts.append(f"{ref_num} ")
                if ref_author_id and str(ref_author_id).strip():
                    header_parts.append(f"(by {ref_author_id})")
                header_parts.append(":\n")
                self.post_text_area.insert(
                    tk.END,
                    utils.sanitize_text_for_tkinter("".join(header_parts)),
                    ("quoted_ref_header"),
                )
                self._insert_text_with_clickable_urls(
                    ref_text_content_raw,
                    ("quoted_ref_text_body",),
                    original_df_index,
                    f"qref_{original_df_index}_{ref_idx}",
                )
                self.post_text_area.insert(tk.END, "\n")
            self.post_text_area.insert(tk.END, "\n")
        main_text_content_raw = post.get("Text", "")
        self.post_text_area.insert(
            tk.END, utils.sanitize_text_for_tkinter("Post Text:\n"), ("bold_label")
        )
        self._insert_text_with_clickable_urls(
            main_text_content_raw, (), original_df_index, f"main_{original_df_index}"
        )
        ...
        if show_images:
            ...
            if (
                images_json_data
                and isinstance(images_json_data, list)
                and len(images_json_data) > 0
            ):
                self.post_text_area.insert(
                    tk.END,
                    utils.sanitize_text_for_tkinter(
                    ...
                    if img_filename:
                        ...
                        if os.path.exists(local_image_path):
                            try:
                                ...
                                img_label.bind(
                                    "<Button-1>",
                                    lambda e, path=local_image_path: utils.open_image_external(
                                        path, self.root
                                    ),
                                )
                                ...
                                print(
                                    f"Error loading/displaying image {local_image_path}: {e}"
                                )
                                self.post_text_area.insert(
                                    tk.END,
                                    utils.sanitize_text_for_tkinter(
                                        f"[Error displaying image: {img_filename}]\n"
                                    ),
                                    "image_val",
                                )
                        else:
                            self.post_text_area.insert(
                                tk.END,
                                utils.sanitize_text_for_tkinter(
                                    f"[Image not found: {img_filename}]\n"
                                ),
                                "image_val",
                            )
            else:
                ...
                if img_count_from_data == 0:
                    self.post_text_area.insert(
                        tk.END,
                        utils.sanitize_text_for_tkinter(f"\n\nImage Count: 0\n"),
                        "image_val",
                    )
                else:
                    self.post_text_area.insert(
        ...
        if (
            metadata_link_raw
            and pd.notna(metadata_link_raw)
            and len(str(metadata_link_raw).strip()) > 0
        ):
            actual_metadata_link_str = utils.sanitize_text_for_tkinter(
                str(metadata_link_raw).strip()
            )
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("\nSource Link: "), "bold_label"
            )
            self._insert_text_with_clickable_urls(
                actual_metadata_link_str,
                ("clickable_link_style",),
                original_df_index,
                f"metalink_{original_df_index}",
            )
            self.post_text_area.insert(tk.END, "\n")
        elif post.get("Site") and post.get("Board"):
            site_text = utils.sanitize_text_for_tkinter(post.get("Site", ""))
            board_text = utils.sanitize_text_for_tkinter(post.get("Board", ""))
            self.post_text_area.insert(
                tk.END, utils.sanitize_text_for_tkinter("\nSource: "), "bold_label"
            )
            self.post_text_area.insert(
                tk.END, f"{site_text}/{board_text}\n", "author_val"
            )
        ...
        if (
            metadata_link_raw
            and isinstance(metadata_link_raw, str)
            and metadata_link_raw.strip()
        ):
            urls_to_scan_for_articles.append(metadata_link_raw.strip())
        if main_text_content_raw:
            urls_to_scan_for_articles.extend(
                utils._extract_urls_from_text(main_text_content_raw)
            )
        ...
        for url in unique_urls_for_article_check:
            if (
                ...
                continue
            if utils.is_excluded_domain(url, config.EXCLUDED_LINK_DOMAINS):
                continue
            exists, filepath = utils.check_article_exists_util(
                safe_filename_post_id, url
            )
            if exists:
                article_found_path = filepath
                break

        if hasattr(self, "view_article_button"):
            if article_found_path:
                self.view_article_button.config(
                    text="View Saved Article",
                    state=tk.NORMAL,
                    command=lambda p=article_found_path: self.open_downloaded_article(
                        p
                    ),
                )
            else:
                self.view_article_button.config(
                    text="Article Not Saved", state=tk.DISABLED, command=lambda: None
                )

        if hasattr(self, "show_links_button"):
            if self.current_post_urls:
                self.show_links_button.config(state=tk.NORMAL)
            else:
                self.show_links_button.config(state=tk.DISABLED)

        if hasattr(self, "edit_save_note_button"):
            self.edit_save_note_button.config(state=tk.NORMAL)
        if hasattr(self, "note_text_area"):
    ...
    def show_welcome_message(self):
        ...
        self.post_text_area.insert(
            tk.END,
            donation_text_display,
            (
                "welcome_text_tag",
                "clickable_link_style",
                clickable_donation_tag_welcome,
            ),
        )
        self.post_text_area.tag_bind(
            clickable_donation_tag_welcome,
            "<Button-1>",
            lambda e, url=donation_url: utils.open_link_with_preference(
                url, self.app_settings
            ),
        )
        self.post_text_area.insert(tk.END, "\n\n", "welcome_text_tag")
    ...
    def update_post_number_label(self, is_welcome=False):  # Added is_welcome
        ...
        post_num_display = (
            f"#{post_num_df}"
            if pd.notna(post_num_df)
            else f"(Original Idx:{original_df_idx})"
        )
        total_in_view = len(self.df_displayed)
        current_pos_in_view = self.current_display_idx + 1
        label_text = (
            f"Result {current_pos_in_view}/{total_in_view} (Q {post_num_display})"
            if self.current_search_active
            else f"Post {current_pos_in_view}/{total_in_view} (Q {post_num_display})"
        )
        self.post_number_label.config(text=label_text)
    ...
    def on_tree_select(self, event):
        ...
        if selected_items:
            ...
            try:
                ...
                if (
                    self.df_displayed is not None
                    and original_df_index in self.df_displayed.index
                ):  # Check df_displayed
                    new_display_idx = self.df_displayed.index.get_loc(original_df_index)
                    if (
                        not hasattr(self, "_init_complete")
                        or not self._init_complete
                        or new_display_idx != self.current_display_idx
                    ):
                        self.current_display_idx = new_display_idx
                        self.update_display()
                    elif (
                        new_display_idx == self.current_display_idx
                ...
                print(
                    f"Error: Tree iid '{selected_iid_str}' not a valid integer for index."
                )
            except Exception as e:
    ...
    def on_tree_arrow_nav(self, event):
        ...
        elif num_items_in_tree > 0:
            current_logical_idx_in_tree = (
                0 if event.keysym == "Down" else num_items_in_tree - 1
            )
        if current_logical_idx_in_tree == -1:
            return "break"
    ...
    def prev_post(self):
        if (
            self.df_displayed is None
            or self.df_displayed.empty
            or len(self.df_displayed) == 0
        ):
            return
        if len(self.df_displayed) == 1 and self.current_display_idx == 0:
            return
        self.current_display_idx = (
            self.current_display_idx - 1 + len(self.df_displayed)
        ) % len(self.df_displayed)
    ...
    def next_post(self):
        ...
        if len(self.df_displayed) == 1 and self.current_display_idx == 0:
            return
        self.current_display_idx = (self.current_display_idx + 1) % len(
            self.df_displayed
        )
        self.select_tree_item_by_idx(self.current_display_idx)
    ...
    def select_tree_item_by_idx(self, display_idx_in_current_df):
        if self.df_displayed is not None and 0 <= display_idx_in_current_df < len(
            self.df_displayed
        ):  # Check df_displayed
            original_df_idx_to_select = self.df_displayed.index[
                display_idx_in_current_df
            ]
    ...
    def restore_placeholder(self, event, placeholder_text, widget=None):
        if widget is None and event:
            ...
            if placeholder_text == config.PLACEHOLDER_POST_NUM:
                ...
                return
        if widget and hasattr(widget, "get") and not widget.get():
            widget.insert(0, placeholder_text)
            placeholder_color = (
                self.placeholder_fg_color_dark
                if self.current_theme == "dark"
                else self.placeholder_fg_color_light
            )
    ...
    def search_post_by_number(self):
        ...
        if search_input == placeholder or not search_input.strip():
            messagebox.showwarning(
                "Input",
                "Please enter a post number, range (e.g., 10-15), or list (e.g., 10,12,15).",
                parent=self.root,
            )
            return
        target_post_numbers = []
        ...
        try:
            ...
            if not target_post_numbers:
                messagebox.showerror(
                    "Input Error", "No valid post numbers to search.", parent=self.root
                )
                return
            if is_jump_to_single:
                ...
                if self.current_search_active:
                    ...
                    self.current_search_active = False
                    self.clear_search_button.config(state=tk.DISABLED)
                    self.repopulate_treeview(self.df_displayed)
                    self.root.update_idletasks()
                matching_posts = self.df_all_posts[
                    self.df_all_posts["Post Number"] == post_to_find
                ]
                if not matching_posts.empty:
                    original_df_idx = matching_posts.index[0]
                    if (
                        self.df_displayed is not None
                        and original_df_idx in self.df_displayed.index
                    ):
                        self.current_display_idx = self.df_displayed.index.get_loc(
                            original_df_idx
                        )
                        self.select_tree_item_by_idx(self.current_display_idx)
                    else:
                        messagebox.showinfo(
                            "Not Found",
                            f"Post # {post_to_find} not found (internal error).",
                            parent=self.root,
                        )
                else:
                    messagebox.showinfo(
                        "Not Found",
                        f"Post # {post_to_find} not found.",
                        parent=self.root,
                    )
            else:
                results = self.df_all_posts[
                    self.df_all_posts["Post Number"].isin(target_post_numbers)
                ]
                self._handle_search_results(results, search_term_str)
        except ValueError:
            messagebox.showerror(
                "Input Error",
                "Invalid input. Please enter a number, a range (e.g., 10-15), or a comma-separated list.",
                parent=self.root,
            )
        finally:
    ...
    def show_calendar(self):
        ...
        if (
            self.df_displayed is not None
            and not self.df_displayed.empty
            and 0 <= self.current_display_idx < len(self.df_displayed)
        ):  # Check df_displayed
            cur_post_dt = self.df_displayed.iloc[self.current_display_idx].get(
                "Datetime_UTC"
            )
        ...
        cal = Calendar(
            ...
            date_pattern="m/d/yy",
            font="Arial 9",
            background=cal_hdr_bg,
            foreground=cal_fg,
            headersbackground=cal_hdr_bg,
            headersforeground=cal_fg,
            normalbackground=cal_bg,
            weekendbackground=cal_bg,
            normalforeground=cal_fg,
            weekendforeground=cal_fg,
            othermonthbackground=cal_dis_bg,
            othermonthwebackground=cal_dis_bg,
            othermonthforeground=cal_dis_fg,
            othermonthweforeground=cal_dis_fg,
            selectbackground=cal_sel_bg,
            selectforeground=cal_sel_fg,
        ...
        def on_date_selected_from_calendar():
            ...
            top.destroy()
            self._search_by_date_str(selected_date_str)

        ttk.Button(
            top, text="Select Date", command=on_date_selected_from_calendar
        ).pack(pady=5)
    ...
    def _search_by_date_str(self, date_str_from_cal):
        try:
            ...
            results = self.df_all_posts[
                self.df_all_posts["Datetime_UTC"].dt.date == target_date
            ]
            self._handle_search_results(
                results, f"Date = {target_date.strftime('%Y-%m-%d')}"
            )
        except Exception as e:
            messagebox.showerror(
                "Error", f"Date selection error: {e}", parent=self.root
            )
    ...
    def search_by_keyword(self):
        ...
        if not keyword or keyword == placeholder:
            messagebox.showwarning(
                "Input", "Please enter a keyword or theme to search.", parent=self.root
            )
            return
        keyword_lower = keyword.lower()
        results = self.df_all_posts[
            (
                self.df_all_posts["Text"]
                .str.lower()
            ...
            | (
                self.df_all_posts["Themes"].apply(
                    ...
                    and any(keyword_lower in theme.lower() for theme in x)
                )
            )
        ]
    ...
    def _handle_search_results(self, results_df, search_term_str):
        if not results_df.empty:
            ...
            self.repopulate_treeview(
                self.df_displayed, select_first_item=True
            )  # Select first result
            ...
            self.df_displayed = pd.DataFrame(
                columns=(
                    self.df_all_posts.columns if self.df_all_posts is not None else []
                )
            )
            self.current_search_active = True
            self.clear_search_button.config(state=tk.NORMAL)
            self.repopulate_treeview(
                self.df_displayed, select_first_item=False
            )  # Show empty tree
    ...
    def export_displayed_list(self):
        ...
        file_types = (
            ...
            else [("CSV", "*.csv"), ("HTML", "*.html")]
        )
        initial_ext = ".html" if export_format == "HTML" else ".csv"
        ...
        if not cols_to_use:
            messagebox.showerror(
                "Export Error",
                "No valid columns found for export. Check EXPORT_COLUMNS.",
                parent=self.root,
            )
            return
            ...
            if final_filename.endswith(".csv"):
                ...
                if "Link" in df_html.columns:
                    df_html["Link"] = df_html["Link"].apply(
                        lambda x: (
                            f'<a href="{html.escape(x, quote=True)}" target="_blank">{html.escape(x)}</a>'
                            if pd.notna(x) and x
                            else ""
                        )
                    )
                if "Text" in df_html.columns:
                    df_html["Text"] = df_html["Text"].apply(
                        utils.format_cell_text_for_gui_html
                    )
                if "Themes" in df_html.columns:
                    df_html["Themes"] = df_html["Themes"].apply(
                        lambda x: (
                            ...
                            else html.escape(str(x))
                        )
                    )
                if "Referenced Posts Display" in df_html.columns:
                    df_html["Referenced Posts Display"] = (
                        df_html["Referenced Posts Display"]
                        .astype(str)
                        .apply(lambda x: x.replace("\n", "<br />\n"))
                    )
                if (
                    "Datetime_UTC" in df_html.columns
                    and pd.api.types.is_datetime64_any_dtype(df_html["Datetime_UTC"])
                ):
                    df_html["Datetime_UTC"] = df_html["Datetime_UTC"].dt.strftime(
                        "%Y-%m-%d %H:%M:%S %Z"
                    )
                if "ImagesJSON" in df_html.columns:
                    df_html["ImagesJSON"] = df_html["ImagesJSON"].apply(
                        lambda x: html.escape(str(x)) if isinstance(x, list) else ""
                    )
                html_table = df_html.to_html(
                    escape=False,
                    index=False,
                    border=0,
                    classes="qposts_table",
                    na_rep="",
                )
                ...
                try:
                    webbrowser.open_new_tab(
                        f"file://{os.path.realpath(final_filename)}"
                    )
                except Exception as e:
                ...
                messagebox.showerror(
                    "Error",
                    "Unsupported file extension. Please use .csv or .html.",
                    parent=self.root,
                )
        except Exception as e:
            messagebox.showerror(
                "Export Error", f"Failed to export: {e}", parent=self.root
            )
            import traceback
    ...
    def toggle_current_post_bookmark(self):
        if (
            ...
            messagebox.showwarning(
                "Bookmark", "No post selected to bookmark/unbookmark.", parent=self.root
            )
            return
        original_df_index = self.df_displayed.index[self.current_display_idx]
        ...
        post_id_str = (
            f"#{post_num_df}"
            if pd.notna(post_num_df)
            else f"(Index: {original_df_index})"
        )
        if original_df_index in self.bookmarked_posts:
            self.bookmarked_posts.remove(original_df_index)
            messagebox.showinfo(
                "Bookmark", f"Q Drop {post_id_str} unbookmarked.", parent=self.root
            )
        else:
            self.bookmarked_posts.add(original_df_index)
            messagebox.showinfo(
                "Bookmark", f"Q Drop {post_id_str} bookmarked!", parent=self.root
            )
        self.update_bookmark_button_status()
        self.update_display()
        self.repopulate_treeview(self.df_displayed)
        self.view_bookmarks_button.config(
            text=f"View Bookmarks ({len(self.bookmarked_posts)})"
        )
    ...
    def update_bookmark_button_status(self, is_welcome=False):  # Added is_welcome
        ...
        original_df_index = self.df_displayed.index[self.current_display_idx]
        self.bookmark_button.config(
            text=(
                "Unbookmark This Post"
                if original_df_index in self.bookmarked_posts
    ...
    def view_bookmarked_gui_posts(self):
        if not self.bookmarked_posts:
            messagebox.showinfo(
                "Bookmarks", "No posts bookmarked yet.", parent=self.root
            )
            return
        valid_bookmarked_indices = [
            idx for idx in self.bookmarked_posts if idx in self.df_all_posts.index
        ]
        if not valid_bookmarked_indices:
            messagebox.showwarning(
                "Bookmarks",
                "Bookmarked posts not found in current data.",
                parent=self.root,
            )
            self.df_displayed = pd.DataFrame(
                columns=(
    ...
    def repopulate_treeview(
        self, dataframe_to_show, select_first_item=True
    ):  # Added select_first_item
        ...
        if dataframe_to_show is not None:
            for original_df_index, row in dataframe_to_show.iterrows():
                ...
                date_str = (
                    date_val_real.strftime("%Y-%m-%d")
                    if pd.notna(date_val_real)
                    else "No Date"
                )
                ...
                post_num_display = (
                    f"#{post_num_real}"
                    if pd.notna(post_num_real)
                    else f"Idx:{iid_original_index_str}"
                )
                is_bookmarked_char = "★" if row.name in self.bookmarked_posts else ""
                self.post_tree.insert(
                    "",
                    "end",
                    iid=iid_original_index_str,
                    values=(post_num_display, date_str, is_bookmarked_char),
                )
        ...
        if dataframe_to_show is not None and not dataframe_to_show.empty:
            if select_first_item:
                ...
                if (
                    hasattr(self, "_init_complete")
                    and self._init_complete
                    and 0 <= self.current_display_idx < len(dataframe_to_show)
                ):
                ...
                iid_to_select_original_index_str = str(
                    dataframe_to_show.index[idx_to_select_in_df]
                )
            ...
            self.current_display_idx = -1
            if (
                hasattr(self, "_init_complete") and self._init_complete
            ):  # If app is running and list becomes empty
                self.show_welcome_message()
        ...
        if not hasattr(self, "_init_complete"):
            self._init_complete = False  # Should already be set in __init__
    ...
    def show_day_delta_dialog(self):
        ...
        month_cb = ttk.Combobox(
            top, textvariable=month_var, values=months, state="readonly", width=18
        )
        month_cb.pack()
        ...
        day_cb = ttk.Combobox(
            top, textvariable=day_var, values=days, state="readonly", width=5
        )
        day_cb.pack()
        ...
        def on_search():
            try:
                ...
                if month_num == 2 and day_num > 29:
                    messagebox.showerror(
                        "Invalid Date",
                        "February cannot have more than 29 days for this search.",
                        parent=top,
                    )
                    return
                    ...
                    messagebox.showerror(
                        "Invalid Date",
                        f"{month_var.get()} cannot have more than 30 days.",
                        parent=top,
                    )
                    return
                elif day_num > 31 or day_num < 1:
                    messagebox.showerror(
                        "Invalid Date", "Day must be between 1 and 31.", parent=top
                    )
                    return
                top.destroy()
                ...
                messagebox.showerror(
                    "Input Error", "Please select a valid month and day.", parent=top
                )
            except Exception as e:
    ...
    def _search_by_month_day(self, month, day):
        if self.df_all_posts is None or "Datetime_UTC" not in self.df_all_posts.columns:
            messagebox.showerror(
                "Error",
                "Post data or 'Datetime_UTC' column not available.",
                parent=self.root,
            )
            return
        if not pd.api.types.is_datetime64_any_dtype(self.df_all_posts["Datetime_UTC"]):
            self.df_all_posts["Datetime_UTC"] = pd.to_datetime(
                self.df_all_posts["Datetime_UTC"], errors="coerce"
            )
            if self.df_all_posts["Datetime_UTC"].isna().any():
                messagebox.showwarning(
                    "Data Warning",
                    "Some dates could not be parsed. Results might be incomplete.",
                    parent=self.root,
                )
        try:
            ...
            self._handle_search_results(
                results, f"Posts from {month_name} {day} (All Years)"
            )
        except Exception as e:
            messagebox.showerror(
                "Search Error",
                f"An error occurred during delta search: {e}",
                parent=self.root,
            )
    ...
    def toggle_note_edit_mode(self):
        ...
        original_df_index = str(
            self.df_displayed.index[self.current_display_idx]
        )  # Use string key
        ...
        if current_state == tk.DISABLED:
            ...
            if note_content:
                ...
                original_df_index in self.user_notes
            ):  # If content is empty, remove note
    ...
    def show_download_window(self):
        if (
            hasattr(self, "download_win")
            and self.download_win is not None
            and self.download_win.winfo_exists()
        ):
        ...
        ttk.Label(
            main_frame, text="Download Post Images", font=("Arial", 11, "bold")
        ).pack(pady=(0, 5), anchor="w")
        ttk.Label(
            main_frame,
            text="Downloads all post image attachments (approx. 400-500MB).",
            wraplength=400,
            justify=tk.LEFT,
        ).pack(anchor="w", padx=5, pady=(0, 5))
        self.download_images_button = ttk.Button(
            main_frame,
            text="Start Image Download",
            command=lambda: self._start_download_thread("images"),
        )
        self.download_images_button.pack(pady=(0, 15), fill="x")
        ttk.Label(
            main_frame, text="Download Linked Articles", font=("Arial", 11, "bold")
        ).pack(pady=(10, 5), anchor="w")
        ttk.Label(
            main_frame,
            text="Downloads HTML from non-board/social links (700MB+ possible).",
            wraplength=400,
            justify=tk.LEFT,
        ).pack(anchor="w", padx=5, pady=(0, 5))
        self.download_articles_button = ttk.Button(
            main_frame,
            text="Start Article Download",
            command=lambda: self._start_download_thread("articles"),
        )
        ...
        self.download_status_label = ttk.Label(
            status_frame, text="Idle.", wraplength=380, justify=tk.LEFT
        )
        self.download_status_label.pack(anchor="w")
        ttk.Button(main_frame, text="Close", command=self.download_win.destroy).pack(
            side="bottom", pady=10
        )

    def _update_download_status(self, message):
        if (
            hasattr(self, "download_status_label")
            and self.download_status_label.winfo_exists()
        ):
    ...
    def _execute_download_task(self, task_name):
        buttons_to_disable = []
        if (
            hasattr(self, "download_images_button")
            and self.download_images_button.winfo_exists()
        ):
            buttons_to_disable.append(self.download_images_button)
        if (
            hasattr(self, "download_articles_button")
            and self.download_articles_button.winfo_exists()
        ):
        ...
        try:
            ...
            if task_name == "images":
                self.root.after(
                    0,
                    lambda: self._update_download_status("Starting Image Download..."),
                )
                utils.download_all_post_images_util(
                    self.df_all_posts,
                    lambda msg: self.root.after(
                        0, lambda m=msg: self._update_download_status(m)
                    ),
                )
                self.root.after(
                    0, lambda: self._update_download_status("Image download finished.")
                )
            elif task_name == "articles":
                self.root.after(
                    0,
                    lambda: self._update_download_status(
                        "Starting Article Download..."
                    ),
                )
                ...
                self.root.after(
                    0,
                    lambda: self._update_download_status("Article download finished."),
                )
            else:
                self.root.after(
                    0,
                    lambda: self._update_download_status(f"Unknown task: {task_name}"),
                )
        except Exception as e:
    ...
    def _start_download_thread(self, task_name):
        if self.df_all_posts is None or self.df_all_posts.empty:
            messagebox.showinfo(
                "No Data",
                "No posts loaded to download content from.",
                parent=(
                    self.download_win
                    if hasattr(self, "download_win")
                    and self.download_win.winfo_exists()
                    else self.root
                ),
            )
        ...
        thread = threading.Thread(
            target=self._execute_download_task, args=(task_name,), daemon=True
        )
        thread.start()
    ...
    def show_settings_window(self):
        if (
            hasattr(self, "settings_win")
            and self.settings_win is not None
        ...
        self.settings_theme_var = tk.StringVar(
            value=self.app_settings.get("theme", settings.DEFAULT_SETTINGS["theme"])
        )
        dark_rb_theme = ttk.Radiobutton(
            theme_frame,
            text="Dark",
        ...
        light_rb_theme = ttk.Radiobutton(
            theme_frame,
            text="Light",
            variable=self.settings_theme_var,
            value="light",
        ...
        link_pref_frame = ttk.Labelframe(
            main_frame, text="Link Opening Preference", padding="10"
        )
        link_pref_frame.pack(fill="x", pady=5)
        self.settings_link_pref_var = tk.StringVar(
            value=self.app_settings.get(
                "link_opening_preference",
                settings.DEFAULT_SETTINGS.get("link_opening_preference", "default"),
            )
        )  # Ensure default for preference
        rb_default = ttk.Radiobutton(
            link_pref_frame,
            text="System Default Browser",
            variable=self.settings_link_pref_var,
            value="default",
            command=self.on_setting_change,
        )
        rb_default.pack(anchor="w", padx=5)
        rb_chrome_incognito = ttk.Radiobutton(
            link_pref_frame,
            text="Google Chrome (Incognito, if available)",
            variable=self.settings_link_pref_var,
            value="chrome_incognito",
            command=self.on_setting_change,
        )
        ...
        ttk.Button(
            close_button_frame, text="Close", command=self.on_settings_window_close
        ).pack(pady=5)
    ...
    def on_setting_change(self, event=None):
        # Theme
        new_theme = (
            self.settings_theme_var.get()
        )  # Semicolon removed, statement on its own line
        theme_changed = False
        ...
        if self.app_settings.get("theme") != new_theme:  # This is the controlling 'if'
            ...
            if hasattr(self, "theme_toggle_button"):
                self.theme_toggle_button.config(
                    text="Into the Dark" if new_theme == "light" else "Dark to Light"
                )

            if (
                hasattr(self, "settings_win")
                and self.settings_win
                and self.settings_win.winfo_exists()
            ):
                try:
                    self.settings_win.configure(
        ...
        if (
            self.app_settings.get("link_opening_preference") != new_link_pref
        ):  # This 'if' should align with the first 'if'
        ...
        if theme_changed or link_pref_changed:
            ...
            print(
                f"Settings saved: Theme='{self.app_settings.get('theme')}', Link Pref='{self.app_settings.get('link_opening_preference')}'"
            )
            if theme_changed:  # Only update full display if theme changed
                if (
                    self.current_display_idx != -1
                    and self.df_displayed is not None
                    and not self.df_displayed.empty
                ):
                    self.update_display()
                else:
    ...
    def show_help_window(self):
        ...
        try:
            ...
            link_fg = (
                self.link_label_fg_dark
                if self.current_theme == "dark"
                else self.link_label_fg_light
            )
        except tk.TclError:
            dialog_bg = "#2b2b2b" if self.current_theme == "dark" else "#f0f0f0"
            bold_font = ("Arial", 12, "bold", "underline")
        ...
        help_win.grab_set()  # Increased height for support section
        main_help_frame = ttk.Frame(help_win, padding="15", style="TFrame")
        main_help_frame.pack(expand=True, fill="both")
        canvas = tk.Canvas(main_help_frame, bg=dialog_bg, highlightthickness=0)
        scrollbar = ttk.Scrollbar(
            main_help_frame, orient="vertical", command=canvas.yview
        )
        ...
        ttk.Label(scrollable_content_frame, text="Help & Tips", font=bold_font).pack(
            pady=(0, 10), anchor="w"
        )
        tips = [
            "- Use the Treeview (left) to navigate posts.",
            "- Click Treeview headers to sort (Post # & Date).",
            ...
            "- 'Today's Deltas' shows posts from this M/D (hover for date).",
            "- Bookmark posts and view them with 'View Bookmarks'.",
            "- Export the current list to HTML or CSV.",
            "- Toggle 'Dark to Light' / 'Into the Dark' for theme.",
            "- Links in post text and 'Show Links' window are clickable.",
        ...
        ttk.Label(scrollable_content_frame, text="Resources", font=bold_font).pack(
            pady=(20, 10), anchor="w"
        )
        resources = {
        ...
        for text, url in resources.items():
            label = ttk.Label(
                scrollable_content_frame, text=text, cursor="hand2", style="TLabel"
            )
            label.configure(
                font=(normal_font[0], normal_font[1], "underline"), foreground=link_fg
            )
            label.pack(anchor="w", padx=5, pady=2)
            label.bind(
                "<Button-1>",
                lambda e, link=url: utils.open_link_with_preference(
                    link, self.app_settings
                ),
            )
        # --- Support & Feedback Section ---
        ttk.Label(
            scrollable_content_frame, text="Support & Feedback", font=bold_font
        ...
        donation_label = ttk.Label(
            scrollable_content_frame,
            text=donation_link_text,
            cursor="hand2",
            style="TLabel",
        )
        donation_label.configure(
            font=(normal_font[0], normal_font[1], "underline"), foreground=link_fg
        )
        donation_label.pack(anchor="w", padx=5, pady=2)
        donation_label.bind(
            "<Button-1>",
            lambda e, link=donation_url: utils.open_link_with_preference(
                link, self.app_settings
            ),
        )
        feedback_email_text = "Feedback/Suggestions: qview1776@gmail.com"
        feedback_label = ttk.Label(
            scrollable_content_frame, text=feedback_email_text, style="TLabel"
        )
        ...
        ttk.Button(close_button_frame, text="Close", command=help_win.destroy).pack(
            pady=(0, 5)
        )
        widgets_to_bind_scroll = [canvas, scrollable_content_frame] + list(
            scrollable_content_frame.winfo_children()
        )
        for widget in widgets_to_bind_scroll:
            widget.bind(
                "<MouseWheel>", lambda e, cw=canvas: self._on_mousewheel(e, cw), add="+"
            )
            widget.bind(
                "<Button-4>", lambda e, cw=canvas: self._on_scroll_up(e, cw), add="+"
            )
            widget.bind(
                "<Button-5>", lambda e, cw=canvas: self._on_scroll_down(e, cw), add="+"
            )
    ...
    def show_post_links_window_external(self):
        if not self.current_post_urls:
            messagebox.showinfo(
                "Links", "No URLs found in the current post.", parent=self.root
            )
            return
        ...
        for url in display_urls:
            ...
            def open_url_action_factory(u=url):
                return lambda e: utils.open_link_with_preference(u, self.app_settings)

            link_label = ttk.Label(
                scrollable_frame, text=url, cursor="hand2", style="TLabel"
            )
            link_label.configure(
                font=("TkDefaultFont", 10, "underline"), foreground=link_label_fg
            )
    ...
    def open_downloaded_article(self, filepath):
        if filepath and os.path.exists(filepath):
            ...
            messagebox.showerror(
                "Error",
                "Saved article file not found or path is invalid.",
                parent=self.root,
            )
            self.update_display()
