...
class QPostViewer:
    ...
    def __init__(self, root):
        ...
        self.placeholder_fg_color_dark = "grey"; self.placeholder_fg_color_light = "#757575"
        self.link_label_fg_dark = "#6DAEFF"; self.link_label_fg_light = "#0056b3"
        ...
        self.details_outer_frame.grid_rowconfigure(1, weight=1) # Image display (will contain canvas)
        self.details_outer_frame.grid_rowconfigure(2, weight=0) # Old notes area, now unused

        # --- Treeview for Post List ---
        ...
        self.post_tree.heading("Post #", text="Post #", anchor='w'); self.post_tree.column("Post #", width=70, stretch=tk.NO, anchor='w')
        self.post_tree.heading("Date", text="Date", anchor='w'); self.post_tree.column("Date", width=110, stretch=tk.YES, anchor='w')
        self.post_tree.heading("Bookmarked", text="★", anchor='center'); self.post_tree.column("Bookmarked", width=30, stretch=tk.NO, anchor='center')
        self.post_tree.grid(row=0, column=0, sticky="nswe"); self.scrollbar_y.grid(row=0, column=1, sticky="ns")
        ...
        self.text_area_frame.grid_rowconfigure(0, weight=1); self.text_area_frame.grid_columnconfigure(0, weight=1)
        ...
        self.post_text_area.grid(row=0, column=0, sticky="nswe"); self.post_text_scrollbar.grid(row=0, column=1, sticky="ns")

        # --- Image Display Area (Scrollable) ---
        self.image_display_outer_frame = ttk.Frame(self.details_outer_frame) # New outer frame for fixed height
        self.image_display_outer_frame.grid(row=1, column=0, sticky="nswe", pady=(5,0))
        self.image_display_outer_frame.grid_rowconfigure(0, weight=1)
        self.image_display_outer_frame.grid_columnconfigure(0, weight=1)
        # Set a fixed height for the image display area viewport. Content will scroll within this.
        self.image_display_outer_frame.config(height=200) # Adjust as needed
        self.image_display_outer_frame.grid_propagate(False) # Prevent children from resizing this frame

        self.image_canvas = tk.Canvas(self.image_display_outer_frame, highlightthickness=0)
        self.image_canvas.grid(row=0, column=0, sticky="nswe")
        self.image_scrollbar = ttk.Scrollbar(self.image_display_outer_frame, orient="vertical", command=self.image_canvas.yview)
        self.image_scrollbar.grid(row=0, column=1, sticky="ns")
        self.image_canvas.configure(yscrollcommand=self.image_scrollbar.set)
        self.image_canvas_content_frame = ttk.Frame(self.image_canvas, style="TFrame")
        self.image_canvas_window = self.image_canvas.create_window((0, 0), window=self.image_canvas_content_frame, anchor="nw")

        def _configure_image_canvas_scrollregion(event): self.image_canvas.configure(scrollregion=self.image_canvas.bbox("all"))
        self.image_canvas_content_frame.bind("<Configure>", _configure_image_canvas_scrollregion)
        def _configure_canvas_window_width(event): self.image_canvas.itemconfig(self.image_canvas_window, width=event.width)
        self.image_canvas.bind("<Configure>", _configure_canvas_window_width)

        image_scroll_widgets = [self.image_canvas, self.image_canvas_content_frame]
        for widget in image_scroll_widgets:
            widget.bind("<MouseWheel>", lambda e, cw=self.image_canvas: self._on_mousewheel(e, cw), add="+")
            widget.bind("<Button-4>", lambda e, cw=self.image_canvas: self._on_scroll_up(e, cw), add="+")
            widget.bind("<Button-5>", lambda e, cw=self.image_canvas: self._on_scroll_down(e, cw), add="+")
        ...
        # ... (Nav frame, Actions frame, Buttons frames as before) ...
        nav_frame = ttk.Frame(controls_main_frame); nav_frame.pack(pady=(0,5), fill=tk.X)
        self.prev_button = ttk.Button(nav_frame, text="<< Prev", command=self.prev_post, width=8); self.prev_button.pack(side="left", padx=2)
        self.post_number_label = ttk.Label(nav_frame, text="", width=35, anchor="center", font=('Arial', 11, 'bold')); self.post_number_label.pack(side="left", padx=5, expand=True, fill=tk.X)
        self.next_button = ttk.Button(nav_frame, text="Next >>", command=self.next_post, width=8); self.next_button.pack(side="left", padx=2)

        actions_frame = ttk.Labelframe(controls_main_frame, text="Search & Actions", padding=(10,5)); actions_frame.pack(pady=5, fill=tk.X, expand=True)
        search_fields_frame = ttk.Frame(actions_frame); search_fields_frame.pack(fill=tk.X, pady=2)
        self.post_entry = ttk.Entry(search_fields_frame, width=12, font=('Arial', 10)); self.post_entry.pack(side=tk.LEFT, padx=(0,2), expand=True, fill=tk.X)
        ttk.Button(search_fields_frame, text="Go", command=self.search_post_by_number, width=4).pack(side=tk.LEFT, padx=(0,10))
        self.post_entry.bind("<FocusIn>", lambda e, p=config.PLACEHOLDER_POST_NUM: self.clear_placeholder(e, p, self.post_entry)); self.post_entry.bind("<FocusOut>", lambda e, p=config.PLACEHOLDER_POST_NUM: self.restore_placeholder(e, p, self.post_entry)); self.post_entry.bind("<Return>", lambda event: self.search_post_by_number())
        self.keyword_entry = ttk.Entry(search_fields_frame, width=20, font=('Arial', 10)); self.keyword_entry.pack(side=tk.LEFT, padx=(0,2), expand=True, fill=tk.X)
        ttk.Button(search_fields_frame, text="Go", command=self.search_by_keyword, width=4).pack(side=tk.LEFT)
        self.keyword_entry.bind("<FocusIn>", lambda e, p=config.PLACEHOLDER_KEYWORD: self.clear_placeholder(e, p, self.keyword_entry)); self.keyword_entry.bind("<FocusOut>", lambda e, p=config.PLACEHOLDER_KEYWORD: self.restore_placeholder(e, p, self.keyword_entry)); self.keyword_entry.bind("<Return>", lambda event: self.search_by_keyword())

        buttons_frame1 = ttk.Frame(actions_frame); buttons_frame1.pack(fill=tk.X, pady=(5,2))
        ...
        self.todays_deltas_button = ttk.Button(buttons_frame1, text="Today's Deltas", command=self.search_today_deltas); self.todays_deltas_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        def get_todays_deltas_tooltip_text(): return f"Posts from {datetime.datetime.now().strftime('%m/%d')} (all years)"
        Tooltip(self.todays_deltas_button, get_todays_deltas_tooltip_text)

        buttons_frame2 = ttk.Frame(actions_frame); buttons_frame2.pack(fill=tk.X, pady=2)
        self.clear_search_button = ttk.Button(buttons_frame2, text="Show All Posts", command=self.clear_search_and_show_all, state=tk.DISABLED); self.clear_search_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        self.show_links_button = ttk.Button(buttons_frame2, text="Show Links", command=self.show_post_links_window_external, state=tk.DISABLED); self.show_links_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        self.view_article_button = ttk.Button(buttons_frame2, text="Article Not Saved", command=lambda: None, state=tk.DISABLED); self.view_article_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)

        buttons_frame3 = ttk.Frame(actions_frame); buttons_frame3.pack(fill=tk.X, pady=2)
        self.bookmark_button = ttk.Button(buttons_frame3, text="Bookmark This Post", command=self.toggle_current_post_bookmark, state=tk.DISABLED); self.bookmark_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        self.view_bookmarks_button = ttk.Button(buttons_frame3, text=f"View Bookmarks ({len(self.bookmarked_posts)})", command=self.view_bookmarked_gui_posts); self.view_bookmarks_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        self.view_edit_note_button = ttk.Button(buttons_frame3, text="View/Edit Note", command=self.show_note_popup, state=tk.DISABLED); self.view_edit_note_button.pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)

        bottom_buttons_frame = ttk.Frame(controls_main_frame); bottom_buttons_frame.pack(pady=(10,0), fill=tk.X, expand=True)
        self.export_var = tk.StringVar(value="HTML"); ttk.OptionMenu(bottom_buttons_frame, self.export_var, "HTML", "HTML", "CSV").pack(side=tk.LEFT, padx=(0,5), fill=tk.X)
        ...
        self.content_sync_button = ttk.Button(bottom_buttons_frame, text="Content Sync", command=self.show_download_window); self.content_sync_button.pack(side=tk.LEFT, padx=5, fill=tk.X)
        self.settings_button = ttk.Button(bottom_buttons_frame, text="Settings", command=self.show_settings_window); self.settings_button.pack(side=tk.LEFT, padx=5, fill=tk.X)
        self.theme_toggle_button = ttk.Button(bottom_buttons_frame, text="Dark to Light", command=self.toggle_theme); self.theme_toggle_button.pack(side=tk.LEFT, padx=5, fill=tk.X)
        self.help_button = ttk.Button(bottom_buttons_frame, text="Help & Info", command=self.show_help_window); self.help_button.pack(side=tk.LEFT, padx=5, fill=tk.X)
        self.about_button = ttk.Button(bottom_buttons_frame, text="About", command=self.show_about_dialog); self.about_button.pack(side=tk.LEFT, padx=5, fill=tk.X)
        ...
        if self.df_all_posts is None or self.df_all_posts.empty:
            messagebox.showerror("Critical Error", "Failed to load any post data. QView cannot continue.")
            self.show_welcome_message() # Show something even if erroring out before destroy
            self.root.destroy()
            return

        self.df_all_posts = self.df_all_posts.reset_index(drop=True)
        self.df_displayed = self.df_all_posts.copy() # df_displayed for initial tree population

        self.repopulate_treeview(self.df_displayed, select_first_item=False) # Populate tree, don't auto-select
        self.show_welcome_message() # Display welcome message

        self._init_complete = True
    # --- END __INIT__ ---

    # --- START ON_CLOSING ---
    # ... (on_closing method as before) ...
    def on_closing(self):
    ...
    # === START::THEME_TOGGLE ===
    # ... (theme methods as before, ensure no references to old embedded note_text_area for styling) ...
    def apply_dark_theme(self):
        self.current_theme = "dark"; self.style.theme_use('clam')
        bg_color = "#2b2b2b"; fg_color = "#e0e0e0"; entry_bg = "#3c3f41"; button_bg = "#4f4f4f"; button_active_bg = "#6a6a6a"; tree_bg = "#3c3f41"; tree_sel_bg = "#0078D7"; tree_sel_fg = "#ffffff"; heading_bg = "#4f4f4f"; scrollbar_bg = '#4f4f4f'; scrollbar_trough = '#3c3f41'; scrollbar_arrow = '#e0e0e0'
        self.root.configure(bg=bg_color); self.style.configure(".", background=bg_color, foreground=fg_color, font=('Arial', 10)); self.style.configure("TFrame", background=bg_color); self.style.configure("TLabel", background=bg_color, foreground=fg_color, padding=3); self.style.configure("TButton", background=button_bg, foreground=fg_color, padding=5, font=('Arial', 9, 'bold'), borderwidth=1, relief=tk.RAISED); self.style.map("TButton", background=[("active", button_active_bg), ("pressed", "#5a5a5a")], relief=[("pressed", tk.SUNKEN)]); self.style.configure("Treeview", background=tree_bg, foreground=fg_color, fieldbackground=tree_bg, borderwidth=1, relief=tk.FLAT); self.style.map("Treeview", background=[("selected", tree_sel_bg)], foreground=[("selected", tree_sel_fg)]); self.style.configure("Treeview.Heading", background=heading_bg, foreground=fg_color, font=('Arial', 10, 'bold'), relief=tk.FLAT, padding=3); self.style.configure("TEntry", fieldbackground=entry_bg, foreground=fg_color, insertbackground=fg_color, relief=tk.SUNKEN, borderwidth=1); self.style.configure("TLabelframe", background=bg_color, foreground=fg_color, relief=tk.GROOVE, borderwidth=1, padding=5); self.style.configure("TLabelframe.Label", background=bg_color, foreground=fg_color, font=('Arial', 10, 'bold')); self.style.configure("Vertical.TScrollbar", background=scrollbar_bg, troughcolor=scrollbar_trough, arrowcolor=scrollbar_arrow, arrowsize=15, width=15); self.style.map('Vertical.TScrollbar', background=[('active', button_active_bg)]); self.style.configure("TCombobox", fieldbackground=entry_bg, background=button_bg, foreground=fg_color, arrowcolor=fg_color, selectbackground=entry_bg, selectforeground=fg_color); self.style.map("TCombobox", fieldbackground=[("readonly", entry_bg)], background=[("readonly", button_bg)], foreground=[("readonly", fg_color)], lightcolor=[("readonly", button_bg)], darkcolor=[("readonly", button_bg)])
        self.post_text_area.configure(bg=entry_bg, fg=fg_color, insertbackground=fg_color, selectbackground=tree_sel_bg)
        if hasattr(self, 'image_canvas'): self.image_canvas.configure(bg=entry_bg) # Style canvas background
        self.post_text_area.tag_configure("bold_label", foreground="#a9b7c6"); self.post_text_area.tag_configure("post_number_val", foreground="#FFCB6B"); self.post_text_area.tag_configure("date_val", foreground="#A5C25C"); self.post_text_area.tag_configure("author_val", foreground="#B0B0B0"); self.post_text_area.tag_configure("themes_val", foreground="#C39AC9"); self.post_text_area.tag_configure("image_val", foreground="#589DF6"); self.post_text_area.tag_configure("clickable_link_style", foreground=self.link_label_fg_dark); self.post_text_area.tag_configure("bookmarked_header", foreground="#FFD700"); self.post_text_area.tag_configure("quoted_ref_header", foreground="#ABBFD0"); self.post_text_area.tag_configure("quoted_ref_text_body", foreground="#cccccc"); self.post_text_area.tag_configure("welcome_title_tag", foreground="#FFCB6B"); self.post_text_area.tag_configure("welcome_text_tag", foreground="#e0e0e0"); self.post_text_area.tag_configure("welcome_emphasis_tag", foreground="#A5C25C"); self.post_text_area.tag_configure("welcome_closing_tag", foreground="#FFD700")
    ...
    def apply_light_theme(self):
        ...
        self.root.configure(bg=bg_color); self.style.configure(".", background=bg_color, foreground=fg_color, font=('Arial', 10)); self.style.configure("TFrame", background=bg_color); self.style.configure("TLabel", background=bg_color, foreground=fg_color, padding=3); self.style.configure("TButton", background=button_bg, foreground=fg_color, padding=5, font=('Arial', 9, 'bold'), borderwidth=1, relief=tk.RAISED); self.style.map("TButton", background=[("active", button_active_bg), ("pressed", "#c1c1c1")], relief=[("pressed", tk.SUNKEN)]); self.style.configure("Treeview", background=tree_bg, foreground=fg_color, fieldbackground=tree_bg, borderwidth=1, relief=tk.FLAT); self.style.map("Treeview", background=[("selected", tree_sel_bg)], foreground=[("selected", tree_sel_fg)]); self.style.configure("Treeview.Heading", background=heading_bg, foreground=fg_color, font=('Arial', 10, 'bold'), relief=tk.FLAT, padding=3); self.style.configure("TEntry", fieldbackground=entry_bg, foreground=fg_color, insertbackground=fg_color, relief=tk.SUNKEN, borderwidth=1); self.style.configure("TLabelframe", background=bg_color, foreground=fg_color, relief=tk.GROOVE, borderwidth=1, padding=5); self.style.configure("TLabelframe.Label", background=bg_color, foreground=fg_color, font=('Arial', 10, 'bold')); self.style.configure("Vertical.TScrollbar", background=scrollbar_bg, troughcolor=scrollbar_trough, arrowcolor=scrollbar_arrow, arrowsize=15, width=15); self.style.map('Vertical.TScrollbar', background=[('active', '#b1b1b1')]); self.style.configure("TCombobox", fieldbackground=entry_bg, background=button_bg, foreground=fg_color, arrowcolor=fg_color, selectbackground=entry_bg, selectforeground=fg_color); self.style.map("TCombobox", fieldbackground=[("readonly", entry_bg)], background=[("readonly", button_bg)], foreground=[("readonly", fg_color)], lightcolor=[("readonly", button_bg)], darkcolor=[("readonly", button_bg)])
        self.post_text_area.configure(bg=entry_bg, fg=fg_color, insertbackground=fg_color, selectbackground=tree_sel_bg)
        if hasattr(self, 'image_canvas'): self.image_canvas.configure(bg=entry_bg) # Style canvas background
        self.post_text_area.tag_configure("bold_label", foreground="#333333"); self.post_text_area.tag_configure("post_number_val", foreground="#D9534F"); self.post_text_area.tag_configure("date_val", foreground="#5CB85C"); self.post_text_area.tag_configure("author_val", foreground="#555555"); self.post_text_area.tag_configure("themes_val", foreground="#8E44AD"); self.post_text_area.tag_configure("image_val", foreground="#337AB7"); self.post_text_area.tag_configure("clickable_link_style", foreground=self.link_label_fg_light); self.post_text_area.tag_configure("bookmarked_header", foreground="#F0AD4E"); self.post_text_area.tag_configure("quoted_ref_header", foreground="#4A4A4A"); self.post_text_area.tag_configure("quoted_ref_text_body", foreground="#202020"); self.post_text_area.tag_configure("welcome_title_tag", foreground="#D9534F"); self.post_text_area.tag_configure("welcome_text_tag", foreground="#000000"); self.post_text_area.tag_configure("welcome_emphasis_tag", foreground="#8E44AD"); self.post_text_area.tag_configure("welcome_closing_tag", foreground="#5CB85C")
    ...
    def toggle_theme(self):
        ...
        if self.current_display_idx != -1 and self.df_displayed is not None and not self.df_displayed.empty : self.update_display()
        else: self.show_welcome_message() # Refresh welcome message with new theme if it's showing
    # --- END TOGGLE_THEME ---
    # === END::THEME_TOGGLE ===

    # --- START CONFIGURE_TEXT_TAGS ---
    # ... (configure_text_tags method as before, including welcome tags) ...
    ...
    # ... (_prevent_text_edit method as before) ...
    def _prevent_text_edit(self, event):
        if event.state & 0x0004 and event.keysym.lower() == 'c': return
        allowed_nav_keys = ["Left", "Right", "Up", "Down", "Prior", "Next", "Home", "End", "Control_L", "Control_R", "Shift_L", "Shift_R", "Alt_L", "Alt_R", "leftarrow", "rightarrow", "uparrow", "downarrow", "PageUp", "PageDown"]
        if event.keysym in allowed_nav_keys or event.keysym.lower() in ["a"] and event.state & 0x0004: return
        return "break"
    # --- END _PREVENT_TEXT_EDIT ---

    # --- START _INSERT_TEXT_WITH_CLICKABLE_URLS ---
    # ... (_insert_text_with_clickable_urls method as before) ...
    def _insert_text_with_clickable_urls(self, text_content_raw, base_tags_tuple, main_post_original_df_index, link_event_tag_prefix):
        ...
        if not isinstance(text_content, str) or not text_content.strip(): self.post_text_area.insert(tk.END, str(text_content) if pd.notna(text_content) else "", base_tags_tuple if base_tags_tuple else ()); return
        urls_in_segment_all = utils._extract_urls_from_text(text_content)
        for url in urls_in_segment_all:
            if url not in self.current_post_urls: self.current_post_urls.append(url)
        last_end = 0
        for url_match in config.URL_REGEX.finditer(text_content):
            ...
            url = url_match.group(0); clickable_tag_instance = f"{link_event_tag_prefix}_url_{url_match.start()}"; current_tags = list(base_tags_tuple) if base_tags_tuple else []; current_tags.extend(['clickable_link_style', clickable_tag_instance])
            self.post_text_area.insert(tk.END, url, tuple(current_tags)); self.post_text_area.tag_bind(clickable_tag_instance, "<Button-1>", lambda e, u=url: utils.open_link_with_preference(u, self.app_settings)); last_end = end
    ...
    # ... (update_display method with call to utils.get_or_create_thumbnail) ...
    def update_display(self):
        if hasattr(self, 'image_canvas_content_frame'): # Clear canvas content
            for widget in self.image_canvas_content_frame.winfo_children(): widget.destroy()
        ...
        original_df_index = self.df_displayed.index[self.current_display_idx]; post = self.df_all_posts.loc[original_df_index]
        ...
        if images_json_data and isinstance(images_json_data, list) and len(images_json_data) > 0:
            ...
            for img_data in images_json_data:
                img_filename_from_json = img_data.get('file')
                if img_filename_from_json:
                    sanitized_base_filename = utils.sanitize_filename_component(os.path.basename(img_filename_from_json)); original_local_image_path = os.path.join(config.IMAGE_DIR, sanitized_base_filename)
                    if os.path.exists(original_local_image_path):
                        photo = utils.get_or_create_thumbnail(original_local_image_path, config.THUMBNAIL_DIR, size=(250, 250))
                        if photo:
                            img_label = ttk.Label(self.image_canvas_content_frame, image=photo, cursor="hand2"); img_label.image = photo; img_label.pack(pady=2, padx=2, anchor="nw")
                            img_label.bind("<MouseWheel>", lambda e, cw=self.image_canvas: self._on_mousewheel(e, cw), add="+"); img_label.bind("<Button-4>", lambda e, cw=self.image_canvas: self._on_scroll_up(e, cw), add="+"); img_label.bind("<Button-5>", lambda e, cw=self.image_canvas: self._on_scroll_down(e, cw), add="+")
                            img_label.bind("<Button-1>", lambda e, path=original_local_image_path: utils.open_image_external(path, self.root)); self.displayed_images_references.append(photo)
                        else: self.post_text_area.insert(tk.END, utils.sanitize_text_for_tkinter(f"[Error displaying thumbnail for: {sanitized_base_filename}]\n"), "image_val")
                    else: self.post_text_area.insert(tk.END, utils.sanitize_text_for_tkinter(f"[Image not found: {sanitized_base_filename}]\n"), "image_val")
        else:
        ...
        if metadata_link_raw and pd.notna(metadata_link_raw) and len(str(metadata_link_raw).strip()) > 0 : actual_metadata_link_str = utils.sanitize_text_for_tkinter(str(metadata_link_raw).strip()); self.post_text_area.insert(tk.END, utils.sanitize_text_for_tkinter("\nSource Link: "), "bold_label"); self._insert_text_with_clickable_urls(actual_metadata_link_str, ("clickable_link_style",) , original_df_index, f"metalink_{original_df_index}"); self.post_text_area.insert(tk.END, "\n")
        ...
        if hasattr(self, 'view_edit_note_button'): self.view_edit_note_button.config(state=tk.NORMAL) # Enable note button
        if hasattr(self, 'image_canvas') and hasattr(self, 'image_canvas_content_frame'): self.image_canvas_content_frame.update_idletasks(); self.image_canvas.configure(scrollregion=self.image_canvas.bbox("all")); self.image_canvas.yview_moveto(0)
    ...
    def show_welcome_message(self):
        ...
        # Clear image display and other post-specific elements
        if hasattr(self, 'image_canvas_content_frame'): # Clear canvas content
            for widget in self.image_canvas_content_frame.winfo_children():
                widget.destroy()
        self.displayed_images_references = []
        self.current_post_urls = []

        # Define welcome message content
        ...
        donation_url = "https://www.buymeacoffee.com/qview1776" # Your actual link
        feedback_text = "Feedback & Suggestions: qview1776@gmail.com"
        closing_text = "🔒 No paywalls. No locked features. Just a tip jar for the digital battlefield." # <<< 'closing_text' IS DEFINED HERE
        ...
        # Clickable donation link in welcome message
        clickable_donation_tag_welcome = "welcome_donation_link_main" # Unique tag
        ...
        self.post_text_area.insert(tk.END, "\n\n", "welcome_text_tag") # Space after donation link

        self.post_text_area.insert(tk.END, feedback_text + "\n\n", "welcome_text_tag")
        # The next line (which was causing the error) will now work because closing_text is defined:
        # self.post_text_area.insert(tk.END, closing_text + "\n", "welcome_closing_tag")
        # ... (this part should all be correctly indented under the method definition) ...

        self.post_text_area.insert(tk.END, closing_text + "\n", "welcome_closing_tag")
        self.post_text_area.config(state=tk.DISABLED)

        # Update other UI elements for welcome state
        # THIS IS THE SECTION WHERE YOUR ERROR (line 760) LIKELY IS:
        if hasattr(self, 'show_links_button'): 
        ...
        # This 'if' block for the old note_text_area was removed in the full gui.py I sent.
        # If it's still present in your code, it might be a source of confusion or part of the error.
        ...
        if hasattr(self, 'image_canvas') and hasattr(self, 'image_canvas_content_frame'):
            self.image_canvas_content_frame.update_idletasks() 
            self.image_canvas.configure(scrollregion=self.image_canvas.bbox("all"))
    # --- END SHOW_WELCOME_MESSAGE ---

    # --- START UPDATE_POST_NUMBER_LABEL ---
    # ... (update_post_number_label method as before) ...
    def update_post_number_label(self, is_welcome=False):
        if is_welcome: self.post_number_label.config(text="Welcome to QView!"); return
        ...
        post_series = self.df_displayed.iloc[self.current_display_idx]; post_num_df = post_series.get('Post Number'); original_df_idx = self.df_displayed.index[self.current_display_idx]; post_num_display = f"#{post_num_df}" if pd.notna(post_num_df) else f"(Original Idx:{original_df_idx})"; total_in_view = len(self.df_displayed); current_pos_in_view = self.current_display_idx + 1; label_text = f"Result {current_pos_in_view}/{total_in_view} (Q {post_num_display})" if self.current_search_active else f"Post {current_pos_in_view}/{total_in_view} (Q {post_num_display})"; self.post_number_label.config(text=label_text)
    # --- END UPDATE_POST_NUMBER_LABEL ---

    # --- START ON_TREE_SELECT ---
    # ... (on_tree_select method as before) ...
        ...
        if selected_items:
            ...
            try:
                ...
                if self.df_displayed is not None and original_df_index in self.df_displayed.index:
                    ...
                    if not hasattr(self, '_init_complete') or not self._init_complete or new_display_idx != self.current_display_idx: self.current_display_idx = new_display_idx; self.update_display()
                    elif new_display_idx == self.current_display_idx and hasattr(self, '_init_complete') and self._init_complete: self.update_display()
    ...
    # ... (on_tree_arrow_nav method as before) ...
    def on_tree_arrow_nav(self, event):
        if self.df_displayed is None or self.df_displayed.empty: return "break"
        all_tree_iids_str = list(self.post_tree.get_children('')); num_items_in_tree = len(all_tree_iids_str) # Semicolon here is okay for simple assignments
        if num_items_in_tree == 0: return "break"
        current_focus_iid_str = self.post_tree.focus(); current_logical_idx_in_tree = -1 # Semicolon here is okay
        ...
        if event.keysym == "Down":
            ...
            return # This return exits the function if not "Up" or "Down"
        
        # This block executes if the key was "Up" or "Down" (and didn't return early from the 'else')
        if 0 <= target_logical_idx_in_tree < num_items_in_tree:
            target_iid_str = all_tree_iids_str[target_logical_idx_in_tree] # Semicolons here are okay for these simple assignments
            self.post_tree.selection_set(target_iid_str)
            self.post_tree.focus(target_iid_str)
            self.post_tree.see(target_iid_str)
            
        return "break" # This "break" is returned by the on_tree_arrow_nav function
    # --- END ON_TREE_ARROW_NAV ---

    # --- START PREV_POST ---
    # ... (prev_post method as before) ...
    def prev_post(self):
    ...
    # ... (select_tree_item_by_idx method as before) ...
    def select_tree_item_by_idx(self, display_idx_in_current_df):
        if self.df_displayed is not None and 0 <= display_idx_in_current_df < len(self.df_displayed):
    ...
    def search_post_by_number(self):
        ...
        if search_input == placeholder or not search_input.strip():
            ...
            if not search_input.strip(): # If truly empty, restore placeholder
        ...
        try:
            if ',' in search_input:
                ...
                if len(parts) == 2:
                    ...
                    # This is the block where your error on line 549 likely occurs.
                    # Ensure this 'if' and its body are correctly indented under 'if len(parts) == 2:'
                    if not start_num_str or not end_num_str: # THIS IS LIKELY LINE 549
                    ...
                    if start_num <= end_num:
                        ...
                        # Clear entry and restore placeholder before returning
                        if entry_widget.get() != placeholder:
                            entry_widget.delete(0, tk.END)
                            self.restore_placeholder(None, placeholder, entry_widget)
                        return
                else: # This 'else' corresponds to 'if len(parts) == 2:'
                    raise ValueError("Invalid range format - not two parts.")
            else: # This 'else' corresponds to 'elif '-' in search_input:'
            ...
            if is_jump_to_single:
                ...
                if self.current_search_active:
                    ...
                    self.repopulate_treeview(self.df_displayed, select_first_item=True) # Assuming True is default
                ...
                if not matching_posts.empty:
                    ...
                    if self.df_displayed is not None and original_df_idx in self.df_displayed.index:
                         ...
                         messagebox.showinfo("Not Found", f"Post # {post_to_find} not found in the current view (internal error).", parent=self.root)
                else:
            ...
            messagebox.showerror("Input Error", "Invalid input. Please enter a number, a range (e.g., 10-15), or a comma-separated list (e.g., 10,12,15). Ensure range parts are not empty.", parent=self.root)
        finally:
    ...
    # ... (keyword search method as before) ...
    def search_by_keyword(self):
        ...
        results = self.df_all_posts[ (self.df_all_posts['Text'].str.lower().str.contains(keyword_lower, na=False, regex=False)) | (self.df_all_posts['Themes'].apply(lambda x: isinstance(x, list) and any(keyword_lower in theme.lower() for theme in x))) ]
    ...
    # ... (_handle_search_results method as before, calling repopulate with select_first_item=True) ...
    def _handle_search_results(self, results_df, search_term_str):
        if not results_df.empty:
            ...
            if not self.df_displayed.empty:
                if self.current_display_idx != 0 or not self.post_tree.selection(): self.current_display_idx = 0; self.select_tree_item_by_idx(self.current_display_idx)
                else: self.update_display()
        else:
            messagebox.showinfo("Search Results", f"No posts found for: {search_term_str}", parent=self.root)
            self.df_displayed = pd.DataFrame(columns=self.df_all_posts.columns if self.df_all_posts is not None else []); self.current_search_active = True; self.clear_search_button.config(state=tk.NORMAL)
            self.repopulate_treeview(self.df_displayed, select_first_item=False); self.show_welcome_message()
    # --- END _HANDLE_SEARCH_RESULTS ---

    # --- START CLEAR_SEARCH_AND_SHOW_ALL ---
    # ... (clear_search_and_show_all method as before, calling repopulate with select_first_item=True) ...
    def clear_search_and_show_all(self):
        if self.df_all_posts is None or self.df_all_posts.empty: self.show_welcome_message(); return
        ...
        self.repopulate_treeview(self.df_displayed, select_first_item=True)
        if self.df_displayed.empty: self.show_welcome_message()
        elif self.post_tree.selection() == (): self.show_welcome_message() # Should be handled by repopulate's selection
    # --- END CLEAR_SEARCH_AND_SHOW_ALL ---

    # --- START EXPORT_DISPLAYED_LIST ---
    # ... (export_displayed_list method as before) ...
    ...
    # ... (bookmarking methods as before, with is_welcome parameter) ...
    def toggle_current_post_bookmark(self):
        ...
        self.update_bookmark_button_status(); self.update_display(); self.repopulate_treeview(self.df_displayed, select_first_item=False); self.select_tree_item_by_idx(self.current_display_idx); self.view_bookmarks_button.config(text=f"View Bookmarks ({len(self.bookmarked_posts)})")
    def update_bookmark_button_status(self, is_welcome=False):
        if is_welcome or self.df_displayed is None or self.df_displayed.empty or self.current_display_idx < 0: self.bookmark_button.config(text="Bookmark Post", state=tk.DISABLED); return
    ...
    def view_bookmarked_gui_posts(self):
        ...
        if not valid_bookmarked_indices: messagebox.showwarning("Bookmarks", "Bookmarked posts not found in current data.", parent=self.root); self.df_displayed = pd.DataFrame(columns=self.df_all_posts.columns if self.df_all_posts is not None else []); self.repopulate_treeview(self.df_displayed, select_first_item=False); self.show_welcome_message(); return
    ...
    # ... (repopulate_treeview method with select_first_item parameter as implemented) ...
    def repopulate_treeview(self, dataframe_to_show, select_first_item=True):
        self.post_tree.delete(*self.post_tree.get_children())
        if dataframe_to_show is not None: # Loop to populate the tree
            for original_df_index, row in dataframe_to_show.iterrows():
                # ... (actual tree insertion code - make sure this is correctly indented)
        ...
        # Logic for selecting an item or handling empty dataframe
        if dataframe_to_show is not None and not dataframe_to_show.empty: # Line 770 (example)
            # This block is indented under the 'if'
            if select_first_item:
                idx_to_select_in_df = 0
                if hasattr(self, 'current_display_idx') and \
            ...
            # (No 'else' for 'if select_first_item:' is fine if not needed)
        else: # Line 772 (example) - This 'else' aligns with the 'if' on line 770
            # This block is indented under the 'else'
            self.current_display_idx = -1
        ...
        if not hasattr(self, '_init_complete'): self._init_complete = False
    # --- END REPOPULATE_TREEVIEW ---

    # --- START DELTA_SEARCH_LOGIC ---
    # ... (delta search methods as before) ...
    ...
    # --- START USER_NOTES_METHODS (Popup version) ---
    # ... (show_note_popup method as implemented) ...
    def show_note_popup(self):
        if self.df_displayed is None or self.df_displayed.empty or not (0 <= self.current_display_idx < len(self.df_displayed)): messagebox.showwarning("No Post Selected", "Please select a post to view or edit its note.", parent=self.root); return
        original_df_index = str(self.df_displayed.index[self.current_display_idx]); current_note = self.user_notes.get(original_df_index, "")
        note_popup = tk.Toplevel(self.root); note_popup.title(f"Note for Post (Index: {original_df_index})"); note_popup.geometry("500x400"); note_popup.transient(self.root); note_popup.grab_set()
        try: dialog_bg = self.style.lookup("TFrame", "background"); text_bg = self.style.lookup("TEntry", "fieldbackground"); text_fg = self.style.lookup("TEntry", "foreground")
        except tk.TclError: dialog_bg = "#f0f0f0" if self.current_theme == "light" else "#2b2b2b"; text_bg = "#ffffff" if self.current_theme == "light" else "#3c3f41"; text_fg = "#000000" if self.current_theme == "light" else "#e0e0e0"
        note_popup.configure(bg=dialog_bg); popup_main_frame = ttk.Frame(note_popup, padding=10); popup_main_frame.pack(expand=True, fill=tk.BOTH)
        note_text_widget = tk.Text(popup_main_frame, wrap=tk.WORD, height=15, font=("TkDefaultFont", 10), relief=tk.SOLID, borderwidth=1, padx=5, pady=5); note_text_widget.configure(bg=text_bg, fg=text_fg, insertbackground=text_fg); note_text_widget.pack(expand=True, fill=tk.BOTH, pady=(0,10)); note_text_widget.insert(tk.END, current_note); note_text_widget.focus_set()
        button_frame = ttk.Frame(popup_main_frame); button_frame.pack(fill=tk.X)
        def save_and_close():
            note_content = note_text_widget.get(1.0, tk.END).strip()
            if note_content: self.user_notes[original_df_index] = note_content
            elif original_df_index in self.user_notes: del self.user_notes[original_df_index]
            utils.save_user_notes(self.user_notes, config.USER_NOTES_FILE_PATH); print(f"Note for post index {original_df_index} saved."); note_popup.destroy()
        def cancel_and_close(): note_popup.destroy()
        save_button = ttk.Button(button_frame, text="Save Note", command=save_and_close); save_button.pack(side=tk.RIGHT, padx=5)
        cancel_button = ttk.Button(button_frame, text="Cancel", command=cancel_and_close); cancel_button.pack(side=tk.RIGHT)
    ...
    # ... (mousewheel helper methods as implemented) ...
    ...
    def _on_scroll_up(self, event, canvas_widget): canvas_widget.yview_scroll(-1, "units")
    def _on_scroll_down(self, event, canvas_widget): canvas_widget.yview_scroll(1, "units")
    # --- END MOUSEWHEEL_HELPERS_FOR_SCROLLABLE_WINDOWS ---

    # --- START DOWNLOAD_WINDOW_AND_THREADING ---
    # ... (download window methods as implemented) ...
    def show_download_window(self):
        if hasattr(self, 'download_win') and self.download_win is not None and self.download_win.winfo_exists(): self.download_win.lift(); self.download_win.focus_set(); return
        self.download_win = tk.Toplevel(self.root); self.download_win.title("Download Offline Content")
        try: dialog_bg = self.style.lookup("TFrame", "background")
        except tk.TclError: dialog_bg = "#2b2b2b" if self.current_theme == "dark" else "#f0f0f0"
        self.download_win.configure(bg=dialog_bg); self.download_win.geometry("450x300"); self.download_win.transient(self.root); self.download_win.grab_set(); self.download_win.resizable(False, False)
        main_frame = ttk.Frame(self.download_win, padding="15"); main_frame.pack(expand=True, fill="both")
        ttk.Label(main_frame, text="Download Post Images", font=('Arial', 11, 'bold')).pack(pady=(0, 5), anchor='w'); ttk.Label(main_frame, text="Downloads all post image attachments (approx. 400-500MB).", wraplength=400, justify=tk.LEFT).pack(anchor='w', padx=5, pady=(0, 5))
        ...
        ttk.Label(main_frame, text="Download Linked Articles", font=('Arial', 11, 'bold')).pack(pady=(10, 5), anchor='w'); ttk.Label(main_frame, text="Downloads HTML from non-board/social links (700MB+ possible).", wraplength=400, justify=tk.LEFT).pack(anchor='w', padx=5, pady=(0, 5))
    ...
    def _execute_download_task(self, task_name):
        ...
        try:
            ...
            if task_name == "images": self.root.after(0, lambda: self._update_download_status("Starting Image Download...")); utils.download_all_post_images_util(self.df_all_posts, lambda msg: self.root.after(0, lambda m=msg: self._update_download_status(m))); self.root.after(0, lambda: self._update_download_status("Image download finished."))
            elif task_name == "articles": self.root.after(0, lambda: self._update_download_status("Starting Article Download...")); utils.scan_and_download_all_articles_util(self.df_all_posts, lambda msg: self.root.after(0, lambda m=msg: self._update_download_status(m))); self.root.after(0, lambda: self._update_download_status("Article download finished."))
    ...
    # ... (settings window methods as implemented) ...
    ...
    def on_setting_change(self, event=None):
        ...
        # Link Opening Preference (This part should align with the first 'if' block)
    ...
    # ... (on_settings_window_close method as implemented) ...
    ...
    # ... (show_help_window method with updated support section and scroll fixes) ...
    def show_help_window(self):
        ...
        help_win = tk.Toplevel(self.root); help_win.title("QView - Help/Tips"); help_win.configure(bg=dialog_bg); help_win.geometry("550x500"); help_win.transient(self.root); help_win.grab_set()
        ...
        for text, url in resources.items(): label = ttk.Label(scrollable_content_frame, text=text, cursor="hand2", style="TLabel"); label.configure(font=(normal_font[0], normal_font[1], "underline"), foreground=link_fg); label.pack(anchor='w', padx=5, pady=2); label.bind("<Button-1>", lambda e, link=url: utils.open_link_with_preference(link, self.app_settings))
        ttk.Label(scrollable_content_frame, text="Support & Feedback", font=bold_font).pack(pady=(20, 10), anchor='w')
        support_text = "If you find QView helpful, you can show your support and help keep the coffee flowing!"; ttk.Label(scrollable_content_frame, text=support_text, wraplength=460, justify=tk.LEFT, font=normal_font).pack(anchor='w', padx=5, pady=1) # wraplength adjusted
        donation_link_text = "Buy me a coffee ☕"; donation_url = "https://www.buymeacoffee.com/qview1776"; donation_label = ttk.Label(scrollable_content_frame, text=donation_link_text, cursor="hand2", style="TLabel"); donation_label.configure(font=(normal_font[0], normal_font[1], "underline"), foreground=link_fg); donation_label.pack(anchor='w', padx=5, pady=2); donation_label.bind("<Button-1>", lambda e, link=donation_url: utils.open_link_with_preference(link, self.app_settings))
        ...
        close_button_frame = ttk.Frame(main_help_frame, style="TFrame"); close_button_frame.pack(fill=tk.X, pady=(15,0)); ttk.Button(close_button_frame, text="Close", command=help_win.destroy).pack(pady=(0,5))
        widgets_to_bind_scroll = [canvas, scrollable_content_frame] + list(scrollable_content_frame.winfo_children())
        for widget in widgets_to_bind_scroll: widget.bind("<MouseWheel>", lambda e, cw=canvas: self._on_mousewheel(e, cw), add="+"); widget.bind("<Button-4>", lambda e, cw=canvas: self._on_scroll_up(e, cw), add="+"); widget.bind("<Button-5>", lambda e, cw=canvas: self._on_scroll_down(e, cw), add="+")
    # --- END SHOW_HELP_WINDOW ---

    # --- START SHOW_ABOUT_DIALOG ---
    # ... (show_about_dialog method as implemented) ...
    def show_about_dialog(self):
        messagebox.showinfo("About QView", "QView - Post Viewer\n\nVersion: 0.9.0 (Scrollable Images)\nDeveloped for exploring Q posts data.\n\nHappy digging!", parent=self.root)
    # --- END SHOW_ABOUT_DIALOG ---

    # --- START SHOW_POST_LINKS_WINDOW_EXTERNAL ---
    # ... (show_post_links_window_external method as implemented) ...
    ...
    # ... (open_downloaded_article method as implemented) ...
    def open_downloaded_article(self, filepath):
        if filepath and os.path.exists(filepath): file_url = f"file:///{os.path.abspath(filepath)}"; utils.open_link_with_preference(file_url, self.app_settings)
        else: messagebox.showerror("Error", "Saved article file not found or path is invalid.", parent=self.root); self.update_display()
