...
if __name__ == "__main__":
    ...
    if not utils.CHROME_PATH:
        print(f"{utils.TermColors.LIGHT_YELLOW}Note:{utils.TermColors.RESET} Google Chrome path was not automatically found (CHROME_PATH='{utils.CHROME_PATH}').")
        print("The 'Open in Incognito' feature for links will attempt to use the system default browser or may fail if Chrome is not set up correctly.")
    ...
    try:
        ...
        if root.winfo_exists():
            ...
            print(f"{utils.TermColors.LIGHT_RED}Error:{utils.TermColors.RESET} GUI root window was destroyed during initialization (likely a critical data loading error). Application cannot start.")
        ...
        if "can't invoke \"winfo\" command" in str(e) or "application has been destroyed" in str(e):
            print(f"{utils.TermColors.LIGHT_YELLOW}Info:{utils.TermColors.RESET} GUI startup failed or was closed very early.")
        else:
            print(f"{utils.TermColors.LIGHT_RED}TclError during GUI startup:{utils.TermColors.RESET} {e}")
            import traceback
            traceback.print_exc()
        ...
        print(f"{utils.TermColors.LIGHT_RED}Unexpected error during GUI startup:{utils.TermColors.RESET} {e}")
        import traceback
        traceback.print_exc()
