...
def load_settings():
    ...
    if not hasattr(config, 'SETTINGS_FILE_PATH'):
        print("Error: SETTINGS_FILE_PATH not found in config.py. Using default settings.")
    ...
    try:
        if os.path.exists(config.SETTINGS_FILE_PATH):
            ...
            print(f"Settings file not found at {config.SETTINGS_FILE_PATH}. Creating with defaults.")
