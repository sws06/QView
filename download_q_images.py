...
def download_all_post_images(df_path):
    ...
    if 'ImagesJSON' not in df.columns:
        ...
        print("Please ensure your Q_Ninja script's load_or_parse_data function is updated to include it and re-create the pickle.")
        return
    ...
    for index, row in df.iterrows():
        ...
        for img_data in images_in_post:
            if not isinstance(img_data, dict):
                print(f"Warning: Malformed image data in post {post_id_display}: {img_data}")
                continue

            img_filename = img_data.get('file') 
            if not img_filename:
                print(f"Warning: Missing 'file' field in image data for post {post_id_display}: {img_data}")
                continue
            ...
            if not os.path.exists(local_image_path):
                try:
                    print(f"Downloading (Post {post_id_display}): {image_url} to {local_image_path}...")
...
if __name__ == "__main__":
    if not os.path.exists(DATAFRAME_PICKLE_PATH):
        print(f"Pickle file {DATAFRAME_PICKLE_PATH} does not exist. Please run your main Q_Ninja GUI script first to generate it with the 'ImagesJSON' column.")
    else:
